...
d=12
DATA_RATIOS=(10.38 10 9.64 9.27 8.89)
# Hardware
...
for dr in "${DATA_RATIOS[@]}"; do
    PARAM_DESC="dr${dr}d${d}"
    TAG="${SERIES_NAME}_miniseries_${PARAM_DESC}"
    START_TIME=$(date +%s)

    log "Training ${PARAM_DESC}"
    ...
    torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train -- \
        --depth=$d \
        --target-param-data-ratio=$dr \
        --run="${WANDB_RUN}_${PARAM_DESC}" \
        --model-tag="${TAG}" \
